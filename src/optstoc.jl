...
drsom_stopping_criterion(
    ...
    (abs.([state.α₁ state.α₂]) |> maximum <= 1e-20) || (state.fx <= ϵ₁) || (state.ϵ <= ϵ₂) || (k > maxiter)

default_rule_terminate(
    fx, gx, k; ϵ₁::F=SNLDEFAULT_TOL_ϵ₁, ϵ₂::F=SNLDEFAULT_TOL_ϵ₂, maxiter=1000
) where {F<:Real} = (fx <= ϵ₁) || (k > maxiter)
...
function SDRSOM(
    ...
    # hessians
    # Hs(x) = H(x, points, Nxview, nothing, ξₕ, nothing)
